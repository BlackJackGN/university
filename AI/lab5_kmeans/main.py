...
from advancedCluster import SpectralClustering
...
def spectral_2d(X: np.ndarray, cl: np.ndarray, n_cl: int) -> None:
    _, ax = plt.subplots(1, 2)
    ax[0].axis('off')
    ax[1].axis('off')
    ax[0].scatter(X[:, 0], X[:, 1], c=cl, s=40)
    ...
    cl_algo = SpectralClustering(n_cl=n_cl, n_init=10, gamma=50.0)
    labels = cl_algo.fit_predict(X)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--dataset', choices=('gaussians', 'twomoon', 'avengers'), default='twomoon')
    p = parser.parse_args()
    ...
    if p.dataset == 'twomoon':
        X, y = two_moon_dataset(n_samples=500, noise=0.05)
        kmeans_2d(X, y, 2)
